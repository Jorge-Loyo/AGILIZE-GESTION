...
class EmpleadosView(QWidget):
    ...
    def _exportar(self):
        ...
        headers = ["Legajo", "Apellido", "Nombre", "DNI", "CUIL", "Email", "Telefono", "Departamento", "Cargo", "Valor Hora", "Sueldo Mensual", "Estado"]
        rows = []
        for e in empleados:
            rows.append([
                e.legajo or "",
                e.apellido, e.nombre, e.dni or "", e.cuil or "", e.email or "",
                e.telefono or "",
                e.departamento.nombre if e.departamento else "",
                e.cargo.nombre if e.cargo else "",
